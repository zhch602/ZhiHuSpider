...
import re, json, cookielib, urllib
...
class User:
    ...
    def get_education(self):
        ...
        soup = self.soup
        try:
            education = soup.find("span", class_="employment item")["title"]
    ...
    def get_residence(self):
        ...
        soup = self.soup
        try:
            residence = soup.find("span", class_="location item")["title"]
        except AttributeError:
            return 0
        else:
            return residence
    ...
    def get_occupation(self):
        ...
        soup = self.soup
        try:
            occupation = soup.find("span", class_="business item")["title"]
        except AttributeError:
            return 0
        else:
            return occupation
    ...
    def get_job(self):
        ...
        soup = self.soup
        try:
            job = soup.find("span", class_="position item")["title"]
        except AttributeError:
            return 0
        else:
            return job
    ...
    def to_image(self):
        image_url = self.get_head_img_url()
        fileName = self.get_user_id()
        dirName = os.path.split(os.path.realpath(__file__))[0] + "\img"
        if os.path.exists(dirName):
            pass
        else:
            os.mkdir(dirName)
        urllib.urlretrieve(image_url, dirName + "\\" + fileName + ".jpg")

    def get_data_id(self):
        """
            By yannisxu (https://github.com/yannisxu)
            增加获取知乎 data-id 的方法来确定标识用户的唯一性 #24
            (https://github.com/egrcc/zhihu/pull/24)
        """
        if self.user_url == None:
            print "I'm anonymous user."
            return 0
    ...
    def get_asks(self):
        """
            ...
            (https://github.com/egrcc/zhihu/pull/29)
        """
        if self.user_url == None:
            print "I'm anonymous user."
            return
...
class Topic:
    ...
    def get_followers_num(self):
        if self.soup == None:
            self.parser()
        soup = self.soup
        followers_num = int(soup.find("div", class_="zm-topic-side-followers-info").a.strong.string)
        return followers_num
        ...
        if self.url == None:
            ...
            if followers_num == 0:
                ...
                for i in xrange((followers_num - 1) / 20 + 1):
                    if i == 0:
                        user_url_list = soup.find_all("h2", class_="zm-list-content-title")
                        for j in xrange(min(followers_num, 40)):
                            url = 'https://www.zhihu.com' + user_url_list[j].a["href"]
                            name = user_url_list[j].a.string.encode("utf-8")
                            yield User(url, name)
                    else:
                        post_url = follower_url
                        _xsrf = soup.find("input", attrs={'name': '_xsrf'})["value"]
                        offset = (i * 20) + 20
                        if i == 1:
                            start = re.findall('<div class="zm-person-item" id="mi-(.*?)">', r.content, re.S)[39]
                        data = {
                            '_xsrf': _xsrf,
                            'start': start,
                            'offset': 20
                        ...
                        r_post = requests.post(post_url, data=data, headers=header)
                        follower_list = r_post.json()["msg"][1]
                        user_link_list = re.findall('"zm-list-avatar-medium" href="(.*?)"><img', follower_list, re.S)
                        user_name_list = re.findall('<h2 class="zm-list-content-title"><a href=".*?">(.*?)</a>',
                                                    follower_list, re.S)
                        for j in xrange(min(followers_num - i * 20 - 40, 20)):
                            user_link = 'https://www.zhihu.com' + user_link_list[j].encode("utf-8")
                            user_name = user_name_list[j].encode("utf-8")
                            yield User(user_link, user_name)
                        start_list = re.findall('<div class="zm-person-item" id="mi-(.*?)</a></h2>', follower_list,
                                                re.S)
